...
int lock_update_main(rp_app_params_t *params)
{
    ...
    params[108].value = (float)g_lock_reg->sf_jumpC              ; // lock_sf_jumpC
    params[109].value = (float) ((g_lock_reg->sf_config      )& 0x01) ; // lock_sf_start
    params[110].value = (float) ((g_lock_reg->sf_config >> 1 )& 0x01) ; // lock_sf_AfrzO
    params[111].value = (float) ((g_lock_reg->sf_config >> 2 )& 0x01) ; // lock_sf_AfrzI
    params[112].value = (float) ((g_lock_reg->sf_config >> 3 )& 0x01) ; // lock_sf_BfrzO
    params[113].value = (float) ((g_lock_reg->sf_config >> 4 )& 0x01) ; // lock_sf_BfrzI
    params[114].value = (float)g_lock_reg->sf_config             ; // lock_sf_CfrzO
    params[115].value = (float)g_lock_reg->sf_config             ; // lock_sf_CfrzI
    params[116].value = (float)g_lock_reg->signal_sw             ; // lock_signal_sw
    params[117].value = (float)g_lock_reg->signal_i              ; // lock_signal_i
    params[118].value = (float)g_lock_reg->sg_amp1               ; // lock_sg_amp1
    params[119].value = (float)g_lock_reg->sg_amp2               ; // lock_sg_amp2
    params[120].value = (float)g_lock_reg->sg_amp3               ; // lock_sg_amp3
    params[121].value = (float)g_lock_reg->sg_amp_sq             ; // lock_sg_amp_sq
    params[122].value = (float) ((g_lock_reg->lpf_F1      )& 0x0f) ; // lock_lpf_F1_tau
    params[123].value = (float) ((g_lock_reg->lpf_F1 >> 4 )& 0x03) ; // lock_lpf_F1_order
    params[124].value = (float) ((g_lock_reg->lpf_F2      )& 0x0f) ; // lock_lpf_F2_tau
    params[125].value = (float) ((g_lock_reg->lpf_F2 >> 4 )& 0x03) ; // lock_lpf_F2_order
    params[126].value = (float) ((g_lock_reg->lpf_F3      )& 0x0f) ; // lock_lpf_F3_tau
    params[127].value = (float) ((g_lock_reg->lpf_F3 >> 4 )& 0x03) ; // lock_lpf_F3_order
    params[128].value = (float) ((g_lock_reg->lpf_sq      )& 0x0f) ; // lock_lpf_sq_tau
    params[129].value = (float) ((g_lock_reg->lpf_sq >> 4 )& 0x03) ; // lock_lpf_sq_order
    params[130].value = (float)g_lock_reg->error_sw              ; // lock_error_sw
    params[131].value = (float)g_lock_reg->error_offset          ; // lock_error_offset
    params[132].value = (float)g_lock_reg->error                 ; // lock_error
    params[133].value = ((float) ( g_lock_reg->error_mean >= 0 ? g_lock_reg->error_mean : g_lock_reg->error_mean-32 ))/262144  ; // lock_error_mean
    lock_error_var    = ((float) (g_lock_reg->error_std))/32 - pow(params[LOCK_ERROR_MEAN].value,2) ;
    params[134].value = lock_error_var<0 ? -1 : sqrt( lock_error_var )  ; // lock_error_std
    params[135].value = (float)g_lock_reg->gen_mod_phase         ; // lock_gen_mod_phase
    params[136].value = (float)g_lock_reg->gen_mod_phase_sq      ; // lock_gen_mod_phase_sq
    params[137].value = (float)g_lock_reg->gen_mod_hp            ; // lock_gen_mod_hp
    params[138].value = (float)g_lock_reg->gen_mod_sqp           ; // lock_gen_mod_sqp
    params[139].value = (float)g_lock_reg->ramp_A                ; // lock_ramp_A
    params[140].value = (float)g_lock_reg->ramp_B                ; // lock_ramp_B
    params[141].value = (float)g_lock_reg->ramp_step             ; // lock_ramp_step
    params[142].value = (float)g_lock_reg->ramp_low_lim          ; // lock_ramp_low_lim
    params[143].value = (float)g_lock_reg->ramp_hig_lim          ; // lock_ramp_hig_lim
    params[144].value = (float)g_lock_reg->ramp_reset            ; // lock_ramp_reset
    params[145].value = (float)g_lock_reg->ramp_enable           ; // lock_ramp_enable
    params[146].value = (float)g_lock_reg->ramp_direction        ; // lock_ramp_direction
    params[147].value = (float)g_lock_reg->ramp_B_factor         ; // lock_ramp_B_factor
    params[148].value = (float)g_lock_reg->sin_ref               ; // lock_sin_ref
    params[149].value = (float)g_lock_reg->cos_ref               ; // lock_cos_ref
    params[150].value = (float)g_lock_reg->cos_1f                ; // lock_cos_1f
    params[151].value = (float)g_lock_reg->cos_2f                ; // lock_cos_2f
    params[152].value = (float)g_lock_reg->cos_3f                ; // lock_cos_3f
    params[153].value = (float)g_lock_reg->sq_ref_b              ; // lock_sq_ref_b
    params[154].value = (float)g_lock_reg->sq_quad_b             ; // lock_sq_quad_b
    params[155].value = (float)g_lock_reg->sq_phas_b             ; // lock_sq_phas_b
    params[156].value = (float)g_lock_reg->sq_ref                ; // lock_sq_ref
    params[157].value = (float)g_lock_reg->sq_quad               ; // lock_sq_quad
    params[158].value = (float)g_lock_reg->sq_phas               ; // lock_sq_phas
    params[159].value = (float)g_lock_reg->in1                   ; // lock_in1
    params[160].value = (float)g_lock_reg->in2                   ; // lock_in2
    params[161].value = (float)g_lock_reg->out1                  ; // lock_out1
    params[162].value = (float)g_lock_reg->out2                  ; // lock_out2
    params[163].value = (float)g_lock_reg->slow_out1             ; // lock_slow_out1
    params[164].value = (float)g_lock_reg->slow_out2             ; // lock_slow_out2
    params[165].value = (float)g_lock_reg->slow_out3             ; // lock_slow_out3
    params[166].value = (float)g_lock_reg->slow_out4             ; // lock_slow_out4
    params[167].value = (float)g_lock_reg->oscA                  ; // lock_oscA
    params[168].value = (float)g_lock_reg->oscB                  ; // lock_oscB
    lock_freeze_regs();
    params[169].value = (float)g_lock_reg->X_28                  ; // lock_X
    params[170].value = (float)g_lock_reg->Y_28                  ; // lock_Y
    params[171].value = (float)g_lock_reg->F1_28                 ; // lock_F1
    params[172].value = (float)g_lock_reg->F2_28                 ; // lock_F2
    params[173].value = (float)g_lock_reg->F3_28                 ; // lock_F3
    params[174].value = (float)g_lock_reg->sqX_28                ; // lock_sqX
    params[175].value = (float)g_lock_reg->sqY_28                ; // lock_sqY
    params[176].value = (float)g_lock_reg->sqF_28                ; // lock_sqF
    params[177].value = (float)g_lock_reg->cnt_clk               ; // lock_cnt_clk
    params[178].value = (float)g_lock_reg->cnt_clk2              ; // lock_cnt_clk2
    lock_restore_regs();
    params[179].value = (float)g_lock_reg->read_ctrl             ; // lock_read_ctrl
    params[180].value = (float)g_lock_reg->pidA_sw               ; // lock_pidA_sw
    params[181].value = (float)g_lock_reg->pidA_PSR              ; // lock_pidA_PSR
    params[182].value = (float)g_lock_reg->pidA_ISR              ; // lock_pidA_ISR
    params[183].value = (float)g_lock_reg->pidA_DSR              ; // lock_pidA_DSR
    params[184].value = (float)g_lock_reg->pidA_SAT              ; // lock_pidA_SAT
    params[185].value = (float)g_lock_reg->pidA_sp               ; // lock_pidA_sp
    params[186].value = (float)g_lock_reg->pidA_kp               ; // lock_pidA_kp
    params[187].value = (float)g_lock_reg->pidA_ki               ; // lock_pidA_ki
    params[188].value = (float)g_lock_reg->pidA_kd               ; // lock_pidA_kd
    params[189].value = (float)g_lock_reg->pidA_in               ; // lock_pidA_in
    params[190].value = (float)g_lock_reg->pidA_out              ; // lock_pidA_out
    params[191].value = (float) ((g_lock_reg->pidA_ctrl                 )& 0x001) ; // lock_pidA_irst
    params[192].value = (float) ((g_lock_reg->pidA_ctrl            >>1  )& 0x001) ; // lock_pidA_freeze
    params[193].value = (float) ((g_lock_reg->pidA_ctrl            >>2  )& 0x001) ; // lock_pidA_ifreeze
    params[194].value = (float)g_lock_reg->ctrl_A                ; // lock_ctrl_A
    params[195].value = (float)g_lock_reg->pidB_sw               ; // lock_pidB_sw
    params[196].value = (float)g_lock_reg->pidB_PSR              ; // lock_pidB_PSR
    params[197].value = (float)g_lock_reg->pidB_ISR              ; // lock_pidB_ISR
    params[198].value = (float)g_lock_reg->pidB_DSR              ; // lock_pidB_DSR
    params[199].value = (float)g_lock_reg->pidB_SAT              ; // lock_pidB_SAT
    params[200].value = (float)g_lock_reg->pidB_sp               ; // lock_pidB_sp
    params[201].value = (float)g_lock_reg->pidB_kp               ; // lock_pidB_kp
    params[202].value = (float)g_lock_reg->pidB_ki               ; // lock_pidB_ki
    params[203].value = (float)g_lock_reg->pidB_kd               ; // lock_pidB_kd
    params[204].value = (float)g_lock_reg->pidB_in               ; // lock_pidB_in
    params[205].value = (float)g_lock_reg->pidB_out              ; // lock_pidB_out
    params[206].value = (float) ((g_lock_reg->pidB_ctrl                 )& 0x001) ; // lock_pidB_irst
    params[207].value = (float) ((g_lock_reg->pidB_ctrl            >>1  )& 0x001) ; // lock_pidB_freeze
    params[208].value = (float) ((g_lock_reg->pidB_ctrl            >>2  )& 0x001) ; // lock_pidB_ifreeze
    params[209].value = (float)g_lock_reg->ctrl_B                ; // lock_ctrl_B
    params[210].value = (float)g_lock_reg->pidC_sw               ; // lock_pidC_sw
    params[211].value = (float)g_lock_reg->pidC_PSR              ; // lock_pidC_PSR
    params[212].value = (float)g_lock_reg->pidC_ISR              ; // lock_pidC_ISR
    params[213].value = (float)g_lock_reg->pidC_DSR              ; // lock_pidC_DSR
    params[214].value = (float)g_lock_reg->pidC_SAT              ; // lock_pidC_SAT
    params[215].value = (float)g_lock_reg->pidC_sp               ; // lock_pidC_sp
    params[216].value = (float)g_lock_reg->pidC_kp               ; // lock_pidC_kp
    params[217].value = (float)g_lock_reg->pidC_ki               ; // lock_pidC_ki
    params[218].value = (float)g_lock_reg->pidC_kd               ; // lock_pidC_kd
    params[219].value = (float)g_lock_reg->pidC_in               ; // lock_pidC_in
    params[220].value = (float)g_lock_reg->pidC_out              ; // lock_pidC_out
    params[221].value = (float) ((g_lock_reg->pidC_ctrl                 )& 0x001) ; // lock_pidC_irst
    params[222].value = (float) ((g_lock_reg->pidC_ctrl            >>1  )& 0x001) ; // lock_pidC_freeze
    params[223].value = (float) ((g_lock_reg->pidC_ctrl            >>2  )& 0x001) ; // lock_pidC_ifreeze
    params[224].value = (float)g_lock_reg->ctrl_C                ; // lock_ctrl_C
    params[225].value = (float)g_lock_reg->aux_A                 ; // lock_aux_A
    params[226].value = (float)g_lock_reg->aux_B                 ; // lock_aux_B
    params[227].value = (float) (( g_lock_reg->lock_feedback >> 0 ) & 0x01 ) ; // lock_ctrl_aux_lock_now
    params[228].value = (float) (( g_lock_reg->lock_feedback >> 1 ) & 0x01 ) ; // lock_ctrl_aux_launch_lock_trig
    params[229].value = (float) (( g_lock_reg->lock_feedback >> 2 ) & 0x01 ) ; // lock_ctrl_aux_pidC_enable_ctrl
    params[230].value = (float) (( g_lock_reg->lock_feedback >> 3 ) & 0x01 ) ; // lock_ctrl_aux_pidB_enable_ctrl
    params[231].value = (float) (( g_lock_reg->lock_feedback >> 4 ) & 0x01 ) ; // lock_ctrl_aux_pidA_enable_ctrl
    params[232].value = (float) (( g_lock_reg->lock_feedback >> 5 ) & 0x01 ) ; // lock_ctrl_aux_ramp_enable_ctrl
    params[233].value = (float) (( g_lock_reg->lock_feedback >> 6 ) & 0x01 ) ; // lock_ctrl_aux_set_pidC_enable
    params[234].value = (float) (( g_lock_reg->lock_feedback >> 7 ) & 0x01 ) ; // lock_ctrl_aux_set_pidB_enable
    params[235].value = (float) (( g_lock_reg->lock_feedback >> 8 ) & 0x01 ) ; // lock_ctrl_aux_set_pidA_enable
    params[236].value = (float) (( g_lock_reg->lock_feedback >> 9 ) & 0x01 ) ; // lock_ctrl_aux_set_ramp_enable
    params[237].value = (float) (( g_lock_reg->lock_feedback >>10 ) & 0x03 ) ; // lock_ctrl_aux_trig_type
    params[238].value = (float) (( g_lock_reg->lock_feedback >>12 ) & 0x01 ) ; // lock_ctrl_aux_lock_trig_rise
